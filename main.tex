...
Open-source stacks such as ARTIQ~\cite{kasprowicz_artiq_2020}, established in trapped-ion and atomic physics experiments, instead replace the full control stack down to the hardware, and their experiments declare timing and parameters explicitly rather than deriving them from a device configuration~\cite{ndscan}.
...
By decoupling experimental logic from hardware-specific implementation, arbok-driver opens pathways to research questions that are difficult to pursue with conventional measurement software. Its modular architecture scales naturally to larger qubit arrays, where the combinatorial growth of control parameters and calibration routines would otherwise make manual sequence management intractable. The framework's support for adaptive feedback protocols and conditional execution enables exploration of error correction schemes, real-time state classification, and closed-loop tuning strategies at scales beyond what static measurement routines can support. Furthermore, because the design principles are hardware-agnostic, the framework can be extended to other qubit platforms like superconducting circuits, trapped ions, and NV centers, enabling cross-platform comparison of measurement protocols and facilitating the transfer of experimental techniques between research groups working on different physical implementations.
