...
\textbf{Run.}
The compiled program executes on the FPGA within a synchronization loop that suspends execution between iterations until explicitly resumed by the driver. This loop is central to the framework's flexibility: it allows the host to update parameters, evaluate runtime conditions, and trigger data retrieval between successive repetitions without recompilation.

\textbf{Interact.}
During execution, the synchronization loop enables two advanced capabilities. Sub-sequences can register step requirements that gate data acquisition and conditionally skip sequence sections at runtime, supporting heralding and repeat-until-success protocols. Additionally, the GenericTuningInterface streams new parameter values to the FPGA between iterations, enabling adaptive measurements driven by optimizers or feedback algorithms. Both mechanisms operate within the existing program structure and require no modification to the sequence definition.
