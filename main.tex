...
\author[unsw,diraq]{S. Serrano}
\author[unsw,diraq]{E. Vahapoglu}
\author[unsw,diraq]{M. Flax}
\author[unsw,diraq]{A. Laucht}
\author[unsw,diraq]{A. S. Dzurak}
...
\item Validated with \SI{300}{\milli\meter} foundry-fabricated silicon spin qubit devices.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|p{6.5cm}|p{6.5cm}|}
        ...
        \hline
        C1 & Current code version & v2.6.1 \\
        \hline
        C2 & Permanent link to code/repository used for this code version & \url{https://github.com/andncl/arbok_driver/tree/v2.6.1} \\
        \hline
        C3 & Permanent link to Reproducible Capsule & \url{https://github.com/andncl/arbok_driver/blob/v2.6.1/pyproject.toml}\\
        \hline
        ...
        \hline
        C7 & Compilation requirements, operating environments \& dependencies & \url{https://github.com/andncl/arbok_driver/blob/v2.6.1/pyproject.toml} \\
        \hline
        C8 & If available Link to developer documentation/manual & \url{https://github.com/andncl/arbok_driver/tree/v2.6.1/docs}, \url{https://arbok-driver.readthedocs.io/en/v2.6.1/} \\
        \hline
...
A complementary approach is taken by ARTIQ~\cite{kasprowicz_artiq_2020}, an open-source stack spanning Sinara hardware, FPGA gateware, and a compiler that translates a subset of Python into real-time kernels, widely established in trapped-ion and atomic physics experiments.
ARTIQ replaces the entire control stack, whereas arbok-driver adds a layer on top of existing vendor hardware and instrumentation.
ARTIQ experiments also declare their timing and parameters explicitly, even in hierarchical extensions such as ndscan~\cite{ndscan}, while arbok-driver derives the full parameter interface from the sequence hierarchy and device configuration.
...
\begin{lstlisting}[
    ...
    the control hardware with the driver while making every section of the program accessible to sub-sequences},
...
In the two-qubit device pre-screening campaign~\cite{steinacker_industry-compatible_2025}, the measurement was executed repeatedly across multiple devices from the same \SI{300}{\milli\meter} foundry process.
...
stability_map = StabilityMap(stability_meas, 'stability_map', stability_map_2q_conf) # stability_map_8q_conf
...
Listing~\ref{lst:stability_map} shows the measurement instantiation for both cases. The only difference between the two-qubit and eight-qubit measurement is the configuration dictionary passed at construction time on line 7. All downstream behavior, including the structure of the QUA program, the number of active readout channels, and the parameter interface exposed to the user, is determined automatically from that configuration. The sequence program for two qubits, structured according to the template in Listing~\ref{lst:qua_program}, is generated identically in both cases and can be seen in Listing~\ref{lst:qua_stability_map} while the QUA output generated from the 8 qubit configuration is shown in the documentation (Tut.~1)~\cite{arbok_docs}. 
...
\end{instructions}
\noindent
The arbok-driver framework fundamentally changes how experimentalists interact with FPGA-based control hardware. Rather than writing, debugging, and maintaining low-level pulse sequence code for each new experiment, users compose measurements from modular Python building blocks and supply device-specific configurations. This shifts the experimentalist's role from hardware programmer to measurement designer: the focus moves from managing timing constraints and register-level details to defining the physics of the experiment. Since all relevant pulse sequence parameters are automatically exposed through the QCoDeS interface, new team members can develop and run measurement routines without requiring direct knowledge of the underlying FPGA programming language. In practice, new users can install the package and begin acquiring data within tens of minutes by reusing existing sub-sequences and experiment blueprints, rather than writing measurement routines from scratch. Because sequence definitions and configurations are shared across projects, measurements are guaranteed to execute identically regardless of which setup or operator runs them. This reproducibility is particularly valuable in growing research teams where engineers with varying levels of hardware expertise need to contribute to complex measurement workflows.

The framework has been adopted internally at Diraq a silicon quantum computing startup and its associated research group at the University of New South Wales, where it is currently deployed across six independent measurement campaigns spanning mass device characterization, qubit array scale-up, benchmarking, novel qubit architectures, and automated tuning with machine learning protocols. It contributed to the tuning and coherent control of an eight-qubit linear array fabricated in a \SI{300}{\milli\meter} CMOS-compatible foundry process, the largest number of qubits reported to date in this platform~\cite{nickl_eight-qubit_2025}, and supported device pre-screening from the same process, contributing to the demonstration of single- and two-qubit gate fidelities exceeding 99\%~\cite{steinacker_industry-compatible_2025}. These results demonstrate that flexible, reproducible control software is a prerequisite for translating high-fidelity qubit performance from academic settings to industry-compatible fabrication processes.

By decoupling experimental logic from hardware-specific implementation, arbok-driver opens pathways to research questions that are difficult to pursue with conventional measurement software. Its modular architecture scales naturally to larger qubit arrays, where the combinatorial growth of control parameters and calibration routines would otherwise make manual sequence management intractable. The framework's support for adaptive feedback protocols and conditional execution enables exploration of error correction schemes, real-time state classification, and closed-loop tuning strategies at scales beyond what static measurement routines can support. Furthermore, because the design principles are hardware-agnostic, the framework can be extended to other qubit platforms — including superconducting circuits, trapped ions, and NV centers — enabling cross-platform comparison of measurement protocols and facilitating the transfer of experimental techniques between research groups working on different physical implementations.
...
\textbf{Andrew S. Dzurak}: Funding acquisition, Supervision.
...
\begin{figure}[H]
\begin{lstlisting}[
    caption={Output of the stability map measurement for a two qubit system defined in listing \ref{lst:stability_map}. An extended version of the measurement for 8 qubits can be found in Tut. 1 of the documentation \cite{arbok_docs}},
