...
\item Dynamically generated QCoDeS instrument interface tailored to each measurement at build time.
\item Modular, device-agnostic measurement sequences scale via declarative configurations.
\item Synchronization loop enables live parameter updates and adaptive feedback without FPGA recompilation.
\item Asynchronous step requirements support heralding and repeat-until-success protocols natively.
\item Validated with \SI{300}{mm} foundry-fabricated silicon spin qubit devices.
\end{itemize}
...
QCoDeS is a Python-based framework for instrument control and data acquisition in experimental physics, developed by Microsoft's quantum computing consortium.
It provides a standardized interface for implementing and using instrument drivers.
To address the challenges outlined above, the arbok-driver package was developed as a dynamically generated instrument within this ecosystem.
...
This allows all relevant parameters, such as wait times, voltages, and frequencies, to be automatically exposed to the user through a familiar interface, without requiring direct interaction with the FPGA code itself.

While a future fault-tolerant quantum computer will require embedded cryogenic control hardware\cite{patra_cryo-cmos_2018, hornibrook_cryogenic_2015, xue_cmos-based_2021}, quantum measurements on processors at the scale of tens to hundreds of qubits can still be performed using room-temperature control systems across a wide range of platforms. In this regime, flexible and scalable software abstractions remain essential regardless of the underlying qubit modality.

arbok-driver has been implemented to interface with the Quantum Machines OPX+ and OPX1000 platforms, and while the current implementation targets their QUA programming language~\cite{qua}, it goes beyond vendor-native abstraction layers such as QUAM~\cite{quam} by integrating established instrumentation stacks via QCoDeS and supporting advanced runtime features including asynchronous conditional execution and adaptive parameter streaming. The underlying design principles of modular sequence composition and dynamic parameter exposure are applicable to any setting in which FPGA-based control hardware is used, including superconducting qubits, trapped ions, NV centers, and quantum sensing platforms.

% While a future fault-tolerant quantum computer will likely require embedded cryogenic control hardware to overcome the overhead associated with electrical interconnects, quantum measurements on processors at the scale of tens to hundreds of qubits can still be performed using room-temperature control systems across a wide range of platforms. In this regime, flexible and scalable software abstractions remain essential regardless of the underlying qubit modality. By combining dynamic abstraction with a modular and fully parameterized sequence design, arbok-driver enables the scalable development of complex measurement routines while remaining compatible with standard laboratory instrumentation. This facilitates integration into existing workflows and provides a flexible foundation for advanced applications such as automated tuning, adaptive measurements, and real-time feedback.

% The arbok-driver framework has been implemented to interface with commercially available FPGA-based control platforms, including the Quantum Machines OPX+ and OPX1000. It has been applied in semiconductor spin qubit experiments, supporting the tuning of an eight-qubit device in a SiMOS heterostructure fabricated in a 300 mm foundry process, which is the largest number of qubits reported to date in this platform~\cite{nickl_eight-qubit_2025}. Separately, the package has been used in the pre-screening of devices from the same foundry process, contributing to the demonstration of single- and two-qubit gate fidelities exceeding 99 percent~\cite{steinacker_industry-compatible_2025}. These applications illustrate how flexible, reproducible and scalable control software can support high-performance quantum experiments. While the current implementation targets the QUA programming language of the Quantum Machines platform, the underlying design principles of modular sequence composition and dynamic parameter exposure are broadly applicable to any experimental setting in which FPGA-based control hardware is used, including superconducting qubits, trapped ions, NV centers, and quantum sensing platforms.
...
\begin{figure}[ht]
    ...
    \caption{
        Block diagram of the arbok-driver
    }
...
Readouts are handled by a dedicated ReadSequence class, which inherits from SubSequence and can appear at any point within a measurement, including multiple times across the sequence hierarchy, accommodating experimental protocols that require mid-sequence or repeated readout \cite{jones_mid-circuit_2026}.
Each ReadSequence manages one or more Signal objects and a corresponding set of AbstractReadout instances, both of which are defined through the readouts configuration and therefore reflect the specific physical layout and readout channels of the device under study.
This means the readout structure is not fixed in the software but adapts to each experimental setup, with different devices exposing a different set of signals and readout handlers. Therfore, the structure of read and sub sequences does not need to change between devices, only the configuration.
The AbstractReadout instances are hardware-specific handlers, quantified by that same configuration, that interface with the physical acquisition channels of the FPGA platform, and each exposes its outputs through a set of GettableParameter objects (subclassing qcodes Parameter), representing measurable quantities returned by the hardware, such as demodulated quadratures or integrated voltages.
This allows readout data to flow naturally into the QCoDeS data acquisition pipeline, enabling seamless integration with existing sweep and logging infrastructure.
...
arbok-driver exposes four main capabilities to the user. Measurements are constructed by composing SubSequence and ReadSequence objects into a hierarchy, either programmatically or from a nested dictionary, and device-specific behavior is fully determined by the configuration passed at construction time. Once instantiated, all tunable parameters across the hierarchy are registered as QCoDeS parameters and can be swept, logged, and updated through the standard QCoDeS interface. For protocols that require conditional execution, such as heralding or repeat-until-success state preparation, sub-sequences can register step requirements that gate data acquisition and sequence execution at runtime without modifying the program structure. Finally, the GenericTuningInterface enables adaptive measurements by streaming new parameter values to the FPGA between sequence iterations, allowing optimizers or feedback algorithms to interact with the hardware through a high-level parameter dictionary without direct access to the underlying program.

The QUA program generated from a given sub-sequence hierarchy follows a fixed structure detailed in Listing~\ref{lst:qua_program}, structured around a synchronization loop that suspends execution between iterations until explicitly resumed by the driver. Within each iteration, every sub-sequence in the hierarchy can place logic at semantically distinct points in the program through a set of hook methods, for example to separate classical pre-computation from the core pulse sequence, or to handle post-processing after readout. This staged structure ensures that timing-critical operations are isolated from classical overhead, making efficient use of the available coherence time.
...
\begin{lstlisting}[
    caption={Pseudo QUA program showing how arbok-driver synchronizes 
    the control hardware with the driver},
...
Listing~\ref{lst:stability_map} shows the measurement instantiation for both cases. The only difference between the two-qubit and eight-qubit measurement is the configuration dictionairy passed at construction time on line 12. All downstream behavior, including the structure of the QUA program, the number of active readout channels, and the parameter interface exposed to the user, is determined automatically from that configuration. The sequence program for two qubits, structured according to the template in Listing~\ref{lst:qua_program}, is generated identically in both cases and can be seen in Listing \ref{lst:qua_stability_map} while the QUA output generated from the 8 qubit configuration is shown in the documentation (Tut.~1.5) \cite{arbok_docs}. 
...
For measurements that are run routinely across different devices or experimental configurations, arbok-driver provides the Experiment class as a higher-level blueprint. An Experiment object captures the invariant structure of a measurement while an arbitrarily amount of device-specific components, in this case the initialization and readout configurations, as required arguments. As shown in the lower half of Listing~\ref{lst:cpmg}, the identical CPMG measurement can be constructed from the CpmgExperiment blueprint by providing only those two components, with all internal sequence structure defined by the blueprint itself. This pattern standardizes frequently used measurements across a group or project, reducing the risk of inconsistencies between runs and making it straightforward for new users to deploy established routines on a new device.

\begin{figure}[t]
\begin{lstlisting}[
    caption={Setting up a CPMG measurement manually via a dictionairy and via a pre-defined experiment},
    label={lst:cpmg},
]
...
The framework has been adopted internally at Diraq, a silicon quantum computing startup, where it forms part of the measurement infrastructure for device characterization and qubit tuning. It contributed to the tuning and coherent control of an eight-qubit linear array fabricated in a 300 mm CMOS-compatible foundry process, the largest number of qubits reported to date in this platform~\cite{nickl_eight-qubit_2025}, and supported device pre-screening from the same process, contributing to the demonstration of single- and two-qubit gate fidelities exceeding 99\%~\cite{steinacker_industry-compatible_2025}. These results demonstrate that flexible, reproducible control software is a prerequisite for translating high-fidelity qubit performance from academic settings to industry-compatible fabrication processes.

In summary, arbok-driver improves the reproducibility and scalability of measurement campaigns by encoding experimental logic in modular, versioned Python code. This facilitates systematic parameter sweeps, automated tuning routines, and the reuse of sequence components across different devices and experiments, directly accelerating the pace of research in the intermediate qubit regime.
...
We thank E. Vahapoglu, S. Serrano, M. Candidio, S. Apicella, F. Dalmagioni and H. Vallabhapurapu for user feedback and feature suggestions, and S. Bartee and A. Klopper for feedback on the manuscript. We also acknowledge P. Steinacker, W. Dumoulin Stuyck, and W. Gilbert for helpful discussions and explanations of previously used measurement techniques.
